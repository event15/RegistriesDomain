...
class RegistrySpec extends ObjectBehavior
{
    ...
    {
        $getRegistryTypeValue = $this->getRegistryType();

        $getRegistryTypeValue->shouldBeEqualTo('car');
    }
    ...
    public function it_registryToArray_should_return_an_array()
    {
        ...
        $this->registryToArray()
             ->shouldHaveKey('name');
        $this->registryToArray()
             ->shouldHaveKey('createdAt');
        $this->registryToArray()
             ->shouldHaveKey('positions');
    }
